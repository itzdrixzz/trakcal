  ...
  return (
    <SafeAreaView className="flex-1">
      ...
      <View className="flex-1 items-center justify-center">
        <View>
          {goalOptions.map((option, Index) => (
            <TouchableOpacity
              key={Index}
              className={`py-[25px] w-[340px] mb-[10px] rounded-2xl items-center ${goal === option ? "bg-[#000000]" : "bg-[#f2f2f2]"}`}
              onPress={() => onChange(option)}
            >
              <Text
                className={`${goal === option ? "text-white" : "text-black"} font-semibold`}
              >
                {option}
              </Text>
            </TouchableOpacity>
      ...
      <View className="mb-[50px] mx-[25px]">
        <TouchableOpacity
          disabled={!goal}
          className={`py-[22px] rounded-full items-center ${goal ? "bg-[#000000]" : "bg-gray-300"}`}
          onPress={onNext}
        >
